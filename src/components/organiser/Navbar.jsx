  ...
  useEffect(() => {
    const getUserPhoto = async () => {
      ...
      const { data, error } = await supabase
        .from('employers') // Ensure you're using the correct table
        .select('empphoto')
      ...
      setUserPhotoUrl(data?.empphoto || null); // Set user photo URL or null if not found
    };
          ...
          <li className='relative flex items-center justify-center gap-x-4'>
              ...
              {showDropdown && (
                <div className="absolute left-0 top-[100%] w-40 bg-white border rounded-md shadow-md z-50">
                  <Link to="/organiser/profile" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Profile</Link>
                  <button
